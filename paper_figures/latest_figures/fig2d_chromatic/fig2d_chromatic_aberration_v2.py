"""
Figure 2d (v2) -- Axial chromatic aberration via wavelength-dependent refractive index.

Same simulation and verification as fig2d_chromatic_aberration.py; this version only
restyles the plot to match fig_paper_caustic: no gridlines, a thicker contrasting
nominal-f reference line, capitalized axis labels, and enlarged legend/tick/label text.
...
colour: the blue beam focuses at a shorter distance than the red beam.

Run:
    MPLBACKEND=Agg python fig2d_chromatic_aberration_v2.py
    FIG2D_N=200000 python fig2d_chromatic_aberration_v2.py    # quick check
"""
import os
import sys
...
# Enlarged text sizes (matched to fig_paper_caustic when both are placed at the same
# column width; this figure is physically wider, so the points are bumped accordingly).
LABEL_SIZE = 12
TICK_SIZE = 11
LEGEND_SIZE = 10
...
def main() -> int:
    ...
    ratio = measured_br_shift / predicted_br_shift if predicted_br_shift != 0 else float("nan")
    t2 = 0.5 < ratio < 2.0
    ...
    for lam in WAVELENGTHS_NM:
        ax.plot(SCREEN_Z, r90_per_lambda[lam], "o-", ms=5, lw=1.8,
                color=COLOURS[lam],
                label=f"{lam:.0f} nm, n={N_VALUES[lam]:.4f}")
        ax.axvline(z_best[lam], color=COLOURS[lam], ls="--", lw=1.2, alpha=0.85)
    # Nominal f: thick black dash-dot so it contrasts with the coloured dashed
    # best-focus lines (different colour AND dash pattern).
    ax.axvline(LENS_F, color="black", ls="-.", lw=1.8, alpha=0.85,
               label=f"Nominal f = {LENS_F:.0f} mm")
    ax.axvline(z_green, color="0.3", ls=":", lw=1.4, alpha=0.8,
               label=f"Observed z(green) = {z_green:.2f} mm")
    ax.set_xlabel("Axial position, z (mm)", fontsize=LABEL_SIZE)
    ax.set_ylabel("R90 spot radius (mm)", fontsize=LABEL_SIZE)
    ax.set_yscale("log")
    ...
    out_pdf = os.path.join(out_dir, "fig2d_chromatic_aberration_v2.pdf")
    out_png = os.path.join(out_dir, "fig2d_chromatic_aberration_v2.png")
