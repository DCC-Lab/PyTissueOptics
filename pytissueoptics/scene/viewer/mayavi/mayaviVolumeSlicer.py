...
class MayaviVolumeSlicer(HasTraits):
    ...
    def make_side_view(self, axis_name):
        ...
        outline = mlab.pipeline.outline(self.data_src3d.mlab_source.dataset, figure=scene.mayavi_scene)
        ipw = mlab.pipeline.image_plane_widget(
            outline, plane_orientation="%s_axes" % axis_name, colormap=self._colormap
        )
        if not self._interpolate:
            ipw.ipw.texture_interpolate = "off"
            ipw.ipw.set_input_data(ipw.ipw._get_input())
