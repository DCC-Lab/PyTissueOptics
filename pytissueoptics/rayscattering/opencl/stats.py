...
class Stats:
    def __init__(self, logger, globalVolumeStats=True, min=(-1, -1, 0), max=(1, 1, 0.5), size=(21, 21, 21), opaqueBoundaries=False):
        self.min = min
        self.max = max
        ...
        for i in range(len(logger)):
            self.scoreInVolume(logger[i][0], logger[i][1])
    ...
    def scoreInVolume(self, position, delta):
        ...
        i = int(self.binSizes[0] * (position[0] - self.min[0]) + 0.5)
        j = int(self.binSizes[1] * (position[1] - self.min[1]) + 0.5)
        k = int(self.binSizes[2] * (position[2] - self.min[2]) + 0.5)
