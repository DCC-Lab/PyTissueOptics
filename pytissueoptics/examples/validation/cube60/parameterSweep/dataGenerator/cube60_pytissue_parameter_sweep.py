...
def pytissueoptics_cube60_sweep_simulator(mu_a_values: List[float], mu_s_values: List[float], g_values: List[float], N: int = 1000000, seed: int = 0) -> List[SweepSimResult]:
    ...
    for mu_a in mu_a_values:
        for mu_s in mu_s_values:
            for g in g_values:
                tissue = ScatteringScene([Cube(1, material=ScatteringMaterial(mu_a=mu_a, mu_s=mu_s, g=g, n=1.1), label="cube")])
...
if __name__ == "__main__":
    results = pytissueoptics_cube60_sweep_simulator([10], [2], [0.9], N=100000, seed=2)
    SweepSimResult.save_to_json(results, "../cube60_n=1.1_results.json", software="pytissueoptics")
